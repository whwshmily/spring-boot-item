           ...
           <form action="#" method="post">
           	<div class="user_form"> 
                <div class="uInfo">
                    ...
                    <input type="text" onblur="checkName(this)"  class="text" name="loginName"/>
                    <span id="nameSpan" display ='none'></span>
                </div>
                <div class="uInfo">
                ...
                <div class="uInfo">
                    ...
                    <input type="text" class="text" id="referrerName" onblur="checkReferrerName(this)" />
                    <span id="checkReferrerName"></span>
                </div>
                <div class="uInfo">
                ...
                <div class="uInfo">
                    ...
                    <input type="button" id="button" onclick="registerUser()" value="同意以下协议，提交"/>
                </div>
            </div>     
            </form>
            <script>
                function registerUser() {
                    ...
                    var recommend = $("input:eq(4)").val();
                  if (password != password1 ){
                      alert("两次密码不一致");
                        return;
                  }
                  if ($("#checkReferrerName").html() || $("#nameSpan").html()||!email) {
                      alert("填写信息有误，请检查");
                      return;
                  }
                 $.ajax({
                     ...
                     success:function (data) {
                         if (data == 'success'){
                             window.location.href = "registerOk.jsp"
                         }else {
                             alert("注册失败，请检查信息");
                         }
                     }
                ...
                function checkReferrerName(e) {
                    $("#checkReferrerName").attr("display","none").html("");
                    $.ajax({
                        url:"user/checkName",
                        type:"post",
                        success:function (data) {
                            ...
                            $("#checkReferrerName").attr("display","inline").html("推荐人不存在");
                        }
                    })
                }
                function checkName(e) {
                    $("#nameSpan").attr("display","none").html("");
                    $.ajax({
                        url:"user/checkName",
                        type:"post",
                        success:function (data) {
                            for (var i = 0 ;i < data.length; i++){
                                if ($(e).val() == data[i]){
                                    $("#nameSpan").attr("display","inline").html("用户名已存在");
                                    return;
                                }
                            }
                        }
